        ...
        for (const line of lines) {
          ...
          if (line.startsWith('data: ')) {
            try {
              ...
              // Track parsed data in debug info
              setDebugInfo(prev => ({
                ...prev,
                parsedData: [...prev.parsedData.slice(-9), {
                  timestamp: new Date().toLocaleTimeString(),
                  type: data.type,
                  content: data.type === 'chunk' ? data.content?.substring(0, 30) + '...' : JSON.stringify(data).substring(0, 50) + '...'
                }]
              }));
      ...
      >
        ...
        {showDebugPanel && (
          <Box sx={{ 
            ...
            <Typography variant="h6" gutterBottom>🐛 Streaming Debug Panel</Typography>
            
            <Box sx={{ mb: 2 }}>
              <Typography variant="subtitle2">Connection Status: {debugInfo.connectionStatus}</Typography>
            </Box>

            <Box sx={{ display: 'flex', gap: 2 }}>
              <Box sx={{ flex: 1 }}>
                <Typography variant="subtitle2">Raw Chunks ({debugInfo.chunks.length})</Typography>
                {debugInfo.chunks.map((chunk, i) => (
                  <Box key={i} sx={{ fontSize: '0.8rem', mb: 0.5 }}>
                    <Typography variant="caption" color="text.secondary">
                      {chunk.timestamp} - {chunk.length} chars
                    </Typography>
              ...
              <Box sx={{ flex: 1 }}>
                <Typography variant="subtitle2">Parsed Data ({debugInfo.parsedData.length})</Typography>
